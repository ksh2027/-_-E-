...
while True:
    if K==1:
        ...
        if(len(PData.axis_y1)==500):
            ...
            y_mid=signal.medfilt(PData.axis_y1,5)
            y_fir =signal.lfilter(b/np.sum(abs(b)), 1, y_mid)
            y2f = np.fft.fftshift(np.fft.fft(y_fir))

            for i in range(0,4):
                if (y_fir[496+i]-y_fir[495+i]) <0 :
                    if slope==1:
                        wave=time_point-past_time_point
                        past_time_point = time_point
                        if wave!=0 and (60/wave)<150 and (60/wave)>40:
                            heartrate=round(60*1/wave)
                            if (H==1) and len(HRV)<10 and (abs(wave-np.mean(HRV))<0.15 or len(HRV)==0):
                                HRV.append(wave)
